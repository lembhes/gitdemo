...
status, response_json = API_Calls.post_async_score_api(ASYNC_URL, ASYNC_SCORE_API_DATA , ACCESS_TOKEN_BEARER,CONTENT_TYPE_APPLICATION_JSON)
...
def test_subject_v2_api():
    status, data = API_Calls.subject_api(BASE_URL2, VALID_SUBJECT_DATA_V2, ACCESS_TOKEN_BEARER, CONTENT_TYPE_APPLICATION_JSON)
    assert status == 201
    print(status, data)
...
def test_score_api_with_questionnaire():
    status, score_json_data = API_Calls.score_api(BASE_URL, VALID_SCORE_DATA_WITH_QS, ACCESS_TOKEN_BEARER,
                                                  CONTENT_TYPE_APPLICATION_JSON)
    print(status, score_json_data)
    assert status == 200
    assertion.assert_valid_schema(score_json_data, 'score_schema.json')
...
@pytest.mark.transcription
def test_get_async_score_v2_api_with_access_token_bearer():
...
def test_post_async_transcriptions_api_secondtime():
    status, transcriptions_json_data = API_Calls.post_async_transcriptions_api(ASYNC_URL, ASYNC_TRANSCRIPTION_API_DATA, ACCESS_TOKEN_BEARER,                                                                    CONTENT_TYPE_APPLICATION_JSON)
    print(status, transcriptions_json_data)
    assert status == 200
    assertion.assert_valid_schema(transcriptions_json_data, 'post_transcription.json')
...
def test_get_async_transcription_api_with_access_token_bearer():
    status, get_score_json =  API_Calls.get_async_transcription_api(ASYNC_URL,ACCESS_TOKEN_BEARER,str(get_job_id))
    print(status, get_score_json)
    assert status_code == 200
    assertion.assert_valid_schema(get_score_json, 'get_transcription.json')
...
def test_get_audio_file_transcription_with_access_token_bearer():
    status, response = API_Calls.get_audio_file_transcription(BASE_URL, FILE_PATH_ENCODEDSAVED, ACCESS_TOKEN_BEARER)
    #print(API_Calls.get_audio_file_transcription(BASE_URL, FILE_PATH_ENCODED_SAVED, ACCESS_TOKEN))
    print(FILE_PATH_ENCODEDSAVED)
    print(status, response)
    assert status == 200
...
def test_E2E_reliable_score_US_location():
    ...
    status_code, response_json = API_Calls.post_inference_parameters_api(BASE_URL2, INFERENCE_PARAMETER_DATA, ACCESS_TOKEN_BEARER,CONTENT_TYPE_APPLICATION_JSON)
    ...
    status_code, response_json = API_Calls.put_inference_parameters_api(BASE_URL2,ACCESS_TOKEN_BEARER,INFERENCE_PARAMETER_ID,INFERENCE_PARAMETER_DATA2, CONTENT_TYPE_APPLICATION_JSON)
    print(ACCESS_TOKEN_BEARER)
    ...
    status_code, response_json = API_Calls.post_inference_scorev2_api(BASE_URL2,SCORE_V2_DATA,ACCESS_TOKEN_BEARER,CONTENT_TYPE_APPLICATION_JSON)
    print(ACCESS_TOKEN_BEARER)
    print("POST inference score v2 API result:", status_code, response_json)
    SCOREID = response_json['id']

    #GET inference scores API
    status_code, response_json = API_Calls.get_inference_scorev2_api(BASE_URL2, SCOREID, ACCESS_TOKEN_BEARER,CONTENT_TYPE_APPLICATION_JSON)
    assert  status_code == 200
    print(ACCESS_TOKEN_BEARER)
...
def test_E2E_create_questionnaire_use_case_flow():
    ...
    status,createq_json = API_Calls.post_create_questionnaire_api(BASE_URL,DATA,ACCESS_TOKEN_BEARER,CONTENT_TYPE_APPLICATION_JSON)
    print (ACCESS_TOKEN_BEARER)
    ...
    status,getq_json=API_Calls.get_questionnaire_required_for_measure(BASE_URL,Q_ID,Q_LANGUAGE,ACCESS_TOKEN_BEARER)
    print(ACCESS_TOKEN_BEARER)
    ...
    status, submit_q_response_json = API_Calls.submit_questionnaire_response(BASE_URL,SUBMIT_QUES_RESPONSE_DATA, ACCESS_TOKEN_BEARER )
    print(ACCESS_TOKEN_BEARER)
    print("submit questionnaire api result:", status, submit_q_response_json)
    assert status == 201


def test_E2E_PHQ2_use_case_flow():
    ...
    res = requests.get(url=BASE_URL + GET_QUESTIONNAIRE_BY_MEASURENAME_ENDPOINT.format(measureName=MEASURE_NAME_MF),
                       headers=headers)
    resonse_data =res.json()
    QUESTIONNAIRE_ID = resonse_data['questionnaire']['id']
    QUESTIONNAIRE_LANGUAGES = resonse_data['questionnaire']['languages'][0]

    print(res)
    assert res.status_code == 200

    time.sleep(5)
    SUBMIT_QUES_RESPONSE_DATA1 = {
        "questionnaire": {
            "id": QUESTIONNAIRE_ID,
    ...
    status, submit_q_response_json = API_Calls.submit_questionnaire_response(BASE_URL,SUBMIT_QUES_RESPONSE_DATA1, ACCESS_TOKEN_BEARER )
    Q_RESPONSE_ID = submit_q_response_json['id']
    print(ACCESS_TOKEN_BEARER)
    print("submit ques api result:", status, submit_q_response_json)
    assert status == 201
    time.sleep(5)

#Post inference parameter

    INF_PARAMETER_REQUEST = {
    ...
    status_code, response_json = API_Calls.post_inference_parameters_api(BASE_URL2, INF_PARAMETER_REQUEST, ACCESS_TOKEN,
                                                                             CONTENT_TYPE_APPLICATION_JSON)

    assert status_code == 201
    print("inf parameter api result:", status_code, response_json)
    INFERENCE_PARAMETER_ID = response_json['id']
    time.sleep(5)
...
#Post score
    ...
    POST_INF_SCORE_REQUEST ={
      "inferenceParameterId": INFERENCE_PARAMETER_ID
     }
    status_code, response_json = API_Calls.post_inference_scorev2_api(BASE_URL2, POST_INF_SCORE_REQUEST,
                                                                      ACCESS_TOKEN,
                                                                      CONTENT_TYPE_APPLICATION_JSON)
    print("post inf score v2 api result:", status_code, response_json)
...
#Get score
    ...
    status_code, response_json = API_Calls.get_inference_scorev2_api(BASE_URL2, SCORE_ID, ACCESS_TOKEN,
                                                                     CONTENT_TYPE_APPLICATION_JSON)
    print("get inf score v2 api result:", status_code, response_json)
    assert status_code == 200
